        ...
        try:
            ...
            for key in item:
                ...
                if key.lower().find('date') != -1:
                    commit_attributes[utils.toCamelCase(
                        key)] = None if item[key] is None else time.mktime(datetime.datetime.strptime(item[key], "%a %b %d %H:%M:%S %Y %z").timetuple())
            ...
            commit_serializer = serializers.FullCommitSerializer(
                data=commit_attributes)
